...
>>>>>>> eecc6b6d2ecf299c380c2b5e9a6c56046d8fcd75
-- =====================================================
-- CAMPUS GOVERNANCE SYSTEM - COMPLETE DATABASE SCHEMA
-- Daffodil International University
-- =====================================================
...
-- =====================================================
-- 6. NOTIFICATIONS TABLE (Email/SMS queue)
-- =====================================================
CREATE TABLE IF NOT EXISTS notifications (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    report_id INTEGER NOT NULL,
...
-- Update user stats after user insert
CREATE TRIGGER IF NOT EXISTS update_stats_after_user_insert 
AFTER INSERT ON users
BEGIN
    UPDATE stats SET 
...
-- =====================================================
-- 11. NOTIFICATION TRIGGER (auto-create on status change)
-- =====================================================
CREATE TRIGGER IF NOT EXISTS create_notification_on_status_change 
AFTER UPDATE OF status ON reports
WHEN NEW.status != OLD.status
...
SELECT '📊 Tables: users, departments, tokens, reports, comments, notifications, activity_logs, stats' as info;
SELECT '👥 Total Users: ' || (SELECT COUNT(*) FROM users) as users_count;
SELECT '📝 Total Reports: ' || (SELECT COUNT(*) FROM reports) as reports_count;
SELECT '🔑 Total Tokens: ' || (SELECT COUNT(*) FROM tokens) as tokens_count;
SELECT '💬 Total Comments: ' || (SELECT COUNT(*) FROM comments) as comments_count;
SELECT '📈 Statistics: ' || total_reports || ' total, ' || pending_reports || ' pending, ' || resolved_reports || ' resolved' FROM stats;
