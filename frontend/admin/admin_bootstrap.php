...
{
    if (!isset($_SESSION['admin_flash'])) return null;
...
function admin_format_datetime(?string $value): string
{
    if (!$value) return '—';
    $ts = strtotime($value);
    if ($ts === false) return admin_h($value);
...
function admin_login_user(array $user): void
{
    ...
    $_SESSION['admin_user_photo'] = (string)($user['profile_photo'] ?? $user['photo'] ?? '');
}
...
function admin_sync_from_backend_session(): bool
{
    ...
    $sql = "SELECT u.id, u.full_name, u.email, u.role, COALESCE(d.name,'Administration') AS department_name, COALESCE(u.profile_photo,'') AS profile_photo
            FROM sessions s
...
{
    $stmt = admin_db()->prepare("SELECT u.*, COALESCE(d.name,'Administration') AS department_name
        FROM users u LEFT JOIN departments d ON d.id = u.department_id
        WHERE lower(u.username)=lower(:login) OR lower(u.email)=lower(:login) LIMIT 1");
    $stmt->execute([':login' => trim($login)]);
    $user = $stmt->fetch();
    if (!$user) return null;
    if (!in_array((string)($user['role'] ?? ''), ['admin','faculty'], true)) return null;
    return $user;
}
...
{
    return admin_query_all("SELECT r.id, COALESCE(t.token,'') AS token, r.title, r.category, COALESCE(r.location,'Not specified') AS location,
        r.priority, r.status, r.created_at,
        (SELECT COUNT(*) FROM report_media rm WHERE rm.report_id = r.id) AS media_count
        FROM reports r LEFT JOIN tokens t ON t.id = r.token_id
        ORDER BY datetime(r.created_at) DESC LIMIT $limit");
}

function backend_fetch_priority_queue(int $limit = 6): array
{
    return admin_query_all("SELECT r.id, COALESCE(t.token,'') AS token, r.title, r.priority, r.status, COALESCE(r.location,'Not specified') AS location, r.created_at
        FROM reports r LEFT JOIN tokens t ON t.id = r.token_id
        WHERE r.status IN ('pending','verified','in_progress')
        ORDER BY CASE r.priority WHEN 'critical' THEN 1 WHEN 'high' THEN 2 WHEN 'medium' THEN 3 ELSE 4 END, datetime(r.created_at) DESC
        LIMIT $limit");
}

function backend_fetch_recent_activity(int $limit = 8): array
{
    $rows = admin_query_all("SELECT action, entity_type, entity_id, COALESCE(new_value,'') AS new_value, created_at
        FROM activity_logs ORDER BY datetime(created_at) DESC LIMIT $limit");
    $out = [];
    foreach ($rows as $row) {
        ...
        $out[] = [
            ...
            'description' => trim(sprintf('%s #%d %s', admin_prettify($entityType), $entityId, $newValue !== '' ? '→ ' . admin_prettify($newValue) : '')),
...
{
    $rows = admin_query_all("SELECT category, COUNT(*) AS total FROM reports GROUP BY category ORDER BY total DESC, category ASC");
    $out = [];
    foreach ($rows as $row) {
        $out[] = ['label' => admin_prettify((string)($row['category'] ?? 'Unknown')), 'count' => (int)($row['total'] ?? 0)];
    }
    return $out;
}
...
function backend_fetch_calendar_report_counts(int $year, int $month): array
{
    ...
    $rows = admin_query_all("SELECT date(created_at) AS report_date, COUNT(*) AS total
        FROM reports
        WHERE strftime('%Y-%m', created_at) = :month
        GROUP BY date(created_at)", [':month' => $monthKey]);
    $out = [];
    foreach ($rows as $row) $out[$row['report_date']] = (int)$row['total'];
    return $out;
}

function admin_reports_fetch(array $filters = []): array
{
    $sql = "SELECT r.*, COALESCE(t.token,'') AS token,
...
{
    return admin_query_one("SELECT r.*, COALESCE(t.token,'') AS token, COALESCE(d.name,'') AS department_name, COALESCE(assign.full_name,'') AS assigned_name
        FROM reports r
        LEFT JOIN tokens t ON t.id = r.token_id
        LEFT JOIN departments d ON d.id = r.department_id
        LEFT JOIN users assign ON assign.id = r.assigned_to
        WHERE r.id = :id LIMIT 1", [':id' => $reportId]);
}

function admin_report_media(int $reportId): array
{
    $rows = admin_query_all("SELECT id, file_path, media_type AS file_type, file_name, mime_type, file_size, created_at
        FROM report_media WHERE report_id = :id ORDER BY datetime(created_at) ASC", [':id' => $reportId]);
    foreach ($rows as &$row) {
...
{
    $rows = admin_query_all("SELECT h.id, h.old_status, h.new_status, COALESCE(h.note,'') AS note, h.created_at, COALESCE(u.full_name,'System') AS changed_by_name
        FROM report_status_history h
        LEFT JOIN users u ON u.id = h.changed_by
        WHERE h.report_id = :id ORDER BY datetime(h.created_at) ASC, h.id ASC", [':id' => $reportId]);
    foreach ($rows as &$row) {
...
function admin_report_update_status(int $reportId, string $status, string $note = '', string $resolutionNotes = ''): bool
{
    ...
    $stmt = $db->prepare("UPDATE reports
        SET status = :status,
            resolution_notes = CASE WHEN :status='resolved' AND :resolution_notes <> '' THEN :resolution_notes
    ...
    if ($ok) {
        $stmt2 = $db->prepare("INSERT INTO report_status_history (report_id, old_status, new_status, changed_by, note) VALUES (:rid,:old,:new,:uid,:note)");
        $stmt2->execute([
            ':rid'=>$reportId,
            ':old'=>$report['status'],
            ':new'=>$status,
...
{
    $stmt = admin_db()->prepare("UPDATE reports SET title=:title, description=:description, category=:category, priority=:priority, location=:location, updated_at=CURRENT_TIMESTAMP WHERE id=:id");
    return $stmt->execute([
        ':title'=>$payload['title'] ?? '',
...
function admin_report_mark_spam(int $reportId, bool $isSpam, string $reason=''): bool
{
    ...
    if (!$report) return false;
    $db = admin_db();
    $stmt = $db->prepare("UPDATE reports SET status=:status, spam_score=:score, spam_reason=:reason, updated_at=CURRENT_TIMESTAMP WHERE id=:id");
    $ok = $stmt->execute([
    ...
    if ($ok) {
        $user = admin_user();
        $db->prepare("INSERT INTO report_status_history (report_id, old_status, new_status, changed_by, note) VALUES (:rid,:old,:new,:uid,:note)")
            ->execute([
                ':rid'=>$reportId,
                ':old'=>$report['status'],
...
function admin_report_delete(int $reportId): bool
{
    ...
    try {
        foreach (['report_media','comments','messages','notifications','report_status_history'] as $table) {
            $col = $table === 'notifications' ? 'report_id' : 'report_id';
...
{
    $user = admin_user();
    $stmt = admin_db()->prepare("INSERT INTO comments (report_id, user_id, comment, is_staff_response, is_private) VALUES (:rid,:uid,:comment,1,0)");
    return $stmt->execute([':rid'=>$reportId, ':uid'=>$user['id'] ?: null, ':comment'=>$note]);
}
...
function admin_report_send_message(int $reportId, string $subject, string $message): bool
{
    ...
    if (!$report) return false;
    $user = admin_user();
    $stmt = admin_db()->prepare("INSERT INTO messages (sender_user_id, recipient_token_id, report_id, subject, message, status) VALUES (:uid,:token_id,:rid,:subject,:message,'sent')");
    return $stmt->execute([
        ':uid'=>$user['id'] ?: null,
...
{
    $sql = "SELECT u.*, COALESCE(d.name,'') AS department_name,
...
{
    return admin_query_one("SELECT u.*, COALESCE(d.name,'') AS department_name FROM users u LEFT JOIN departments d ON d.id = u.department_id WHERE u.id=:id", [':id'=>$userId]);
}

function admin_user_update_status(int $userId, string $status): bool
{
    $active = $status === 'blocked' ? 0 : 1;
    $stmt = admin_db()->prepare("UPDATE users SET is_active=:active, updated_at=CURRENT_TIMESTAMP WHERE id=:id");
    return $stmt->execute([':active'=>$active, ':id'=>$userId]);
}

function admin_user_recent_messages(int $userId): array
{
    return admin_query_all("SELECT * FROM messages WHERE recipient_user_id = :id OR sender_user_id = :id ORDER BY datetime(created_at) DESC LIMIT 12", [':id'=>$userId]);
}

function admin_user_send_message(int $userId, string $subject, string $message): bool
{
    $sender = admin_user();
    $stmt = admin_db()->prepare("INSERT INTO messages (sender_user_id, recipient_user_id, subject, message, status) VALUES (:sender,:recipient,:subject,:message,'sent')");
    return $stmt->execute([':sender'=>$sender['id'] ?: null, ':recipient'=>$userId, ':subject'=>$subject, ':message'=>$message]);
}

function admin_message_threads(): array
{
    $sql = "SELECT m.report_id, m.recipient_user_id, m.recipient_token_id, MAX(m.created_at) AS last_at,
            MAX(m.subject) AS last_subject, COUNT(*) AS total_messages,
    ...
    foreach ($rows as $i => $row) $rows[$i]['id'] = $i+1;
    return $rows;
}

function admin_message_thread_items_by_tuple(?int $reportId, ?int $recipientUserId, ?int $recipientTokenId): array
{
    $sql = "SELECT m.*, COALESCE(sender.full_name,'System') AS sender_name
            FROM messages m
            LEFT JOIN users sender ON sender.id = m.sender_user_id
            WHERE COALESCE(m.report_id,0)=:rid AND COALESCE(m.recipient_user_id,0)=:ruid AND COALESCE(m.recipient_token_id,0)=:rtid
            ORDER BY datetime(m.created_at) ASC";
    return admin_query_all($sql, [':rid'=>$reportId ?: 0, ':ruid'=>$recipientUserId ?: 0, ':rtid'=>$recipientTokenId ?: 0]);
}

function admin_message_send(array $payload): bool
{
    $sender = admin_user();
    $stmt = admin_db()->prepare("INSERT INTO messages (sender_user_id, recipient_user_id, recipient_token_id, report_id, subject, message, status)
        VALUES (:sender,:recipient_user_id,:recipient_token_id,:report_id,:subject,:message,'sent')");
    return $stmt->execute([
...
{
    $sql = "SELECT r.*, COALESCE(t.token,'') AS token FROM reports r LEFT JOIN tokens t ON t.id = r.token_id
...
{
    $row = admin_query_one("SELECT u.*, COALESCE(d.name,'Administration') AS department_name
        FROM users u LEFT JOIN departments d ON d.id = u.department_id WHERE u.id = :id", [':id'=>$userId]) ?: [];
    return [
        'id' => $row['id'] ?? $userId,
...
{
    $stmt = admin_db()->prepare("UPDATE users SET full_name=:name, email=:email, phone=:phone, profile_photo=COALESCE(:photo, profile_photo), updated_at=CURRENT_TIMESTAMP WHERE id=:id");
    return $stmt->execute([
        ':name'=>$payload['name'] ?? '',
        ':email'=>$payload['email'] ?? '',
        ':phone'=>$payload['phone'] ?? '',
                ':photo'=>$payload['profile_photo'] ?? null,
        ':id'=>$userId
    ]);
}
...
function admin_profile_change_password(int $userId, string $currentPassword, string $newPassword): bool
{
    ...
    if (!$user || !password_verify($currentPassword, (string)$user['password_hash'])) return false;
    $stmt = admin_db()->prepare("UPDATE users SET password_hash=:hash, updated_at=CURRENT_TIMESTAMP WHERE id=:id");
    return $stmt->execute([':hash'=>password_hash($newPassword, PASSWORD_DEFAULT), ':id'=>$userId]);
}

function backend_profile_store_photo(array $file): ?string
{
    if (empty($file['name']) || !isset($file['tmp_name']) || !is_uploaded_file($file['tmp_name'])) return null;
...
function build_calendar_matrix(int $year, int $month): array
{
    ...
    while ($cursor <= $end) {
        $week = [];
        for ($i=0; $i<7; $i++) { $week[] = $cursor; $cursor = $cursor->modify('+1 day'); }
        $weeks[] = $week;
    }
    return $weeks;
}
